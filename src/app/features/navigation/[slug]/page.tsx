...
type NavigationDetailPageProps = {
  params: Promise<{
    slug: string;
  }>;
};
...
export default async function NavigationDetailPage({
  params,
}: NavigationDetailPageProps) {
  const { slug } = await params;
  const sample = NAVIGATION_SAMPLES.find((item) => item.slug === slug);
  ...
  return (
    <main style={styles.page}>
      <section style={styles.content}>
        <p style={styles.eyebrow}>Navigation sample</p>
...
export function generateStaticParams() {
  return NAVIGATION_SAMPLES.map((sample) => ({ slug: sample.slug }));
}
