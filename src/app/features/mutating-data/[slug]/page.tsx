...
export default async function MutationDetailPage({
  params,
}: MutationDetailPageProps) {
  const { slug } = await params;
  const sample = MUTATION_SAMPLES.find((item) => item.slug === slug);
  ...
  return (
    <main style={styles.page}>
      <section style={styles.content}>
        <p style={styles.eyebrow}>Mutating data sample</p>
...
export function generateStaticParams() {
  return MUTATION_SAMPLES.map((sample) => ({ slug: sample.slug }));
}
